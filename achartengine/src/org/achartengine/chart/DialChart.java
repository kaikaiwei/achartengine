...
public class DialChart extends RoundChart {
  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
    //setAntiAlias  Helper for setFlags(), setting or clearing the ANTI_ALIAS_FLAG bit AntiAliasing
    //smooths out the edges of what is being drawn, 
    //but is has no impact on the interior of the shape.
    paint.setAntiAlias(mRenderer.isAntialiasing());
    paint.setStyle(Style.FILL);//填充
    paint.setTextSize(mRenderer.getLabelsTextSize());//设置文字大小
  ...
  private double getAngleForValue(double value, double minAngle, double maxAngle, double min,
    ...
    return Math.toRadians(minAngle + (value - min) * angleDiff / diff);//得到弧度值
  }

  /**
   * Draws the chart tick lines.
   * 绘制图表的连接线，就是饼图中的说明文字跟类别的连线
  ...
  private void drawNeedle(Canvas canvas, double angle, int centerX, int centerY, double radius,
    ...
    double diff = Math.toRadians(90);//90的弧度值
    //得到正弦余弦变换
    ...
    if (arrow) {
      ...
      //绘制路径
      float width = paint.getStrokeWidth();//得到笔触宽度
